...
```powershell
& "$AA\revisar-inicial.ps1"          -Path "<peça>"   # resíduo, identificador divergente, numeração, requisitos do art. 319
& "$AA\extenso.ps1"                  -Path "<peça>"   # confere cada par "R$ X (extenso)"
& "$AA\validar-identificadores.ps1"  -Path "<peça>"   # dígito verificador de CPF, CNPJ e número CNJ
```
