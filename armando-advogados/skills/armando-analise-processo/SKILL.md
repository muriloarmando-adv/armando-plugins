...
description: Analisa processo judicial já em curso no padrão do escritório Armando Advogados — lê os autos na ordem certa (o extrato está atual? → índice oficial → último ato de juízo → peça inaugural → defesa → prova), fixa o último movimento e o prazo em curso, mapeia as peças e entrega um dos quatro produtos da casa: FICHA RESUMO DO PROCESSO, RESUMO PARA O CLIENTE, PARECER ESTRATÉGICO ou TRIAGEM DE PRAZO. Use SEMPRE que o usuário pedir para "analisar esse processo", "resumir os autos", "ler o processo e me dizer o que tem", "fazer a ficha resumo", "em que pé está", "qual a fase", "quais os últimos movimentos", "tem prazo correndo?", "o que o juiz decidiu", "vale a pena recorrer", "monta a estratégia desse caso", "resumo da reclamação trabalhista", "diagnóstico do caso", "explica esse processo para o cliente" — mesmo que não diga "análise" ou "padrão do escritório". Vale também quando o PDF entregue é o processo: aqui a conversão é só o primeiro passo, e quem conduz é esta skill, não `armando-pdf-markdown`. Para redigir a peça depois de analisar, use `armando-peticao-inicial` (inicial) ou apenas `armando-timbrado` (defesa e recurso). Para contrato, `armando-analise-contrato`.
---
